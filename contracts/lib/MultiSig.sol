...
abstract contract MultiSig {
    mapping(address => bool) internal s_owners;

    address internal s_markedForRemoval;
    ...
    event MultiSigRequest(
        address indexed from,
        bytes4 indexed selector,
        uint256 value,
        bytes32 hashedData
    );
    event MultiSigExecute(
        address indexed from,
        bytes4 indexed selector,
        uint256 value,
        bytes32 hashedData
    ...
    event MultiSigReplaceOwnerCall(
        address indexed by,
        address indexed from,
        address indexed to
    );
    ...
    modifier multiSig2of3(uint256 value) {
        require(
            s_owners[msg.sender] && msg.sender != s_markedForRemoval,
            "MultiSig: only owners that are not being removed"
        );
        ...
        require(
            s_action.owner != msg.sender,
            "MultiSig: same owner cannot sign twice"
        );
        require(s_action.value == value, "MultiSig: must sign the same value");
        require(
            s_action.data == hashedData,
            "MultiSig: must sign the same data"
        );
    ...
    function cancel() external {
        ...
        require(
            s_markedForRemoval != msg.sender,
            "MultiSig: only owners that are not being replaced can cancel"
        );
        s_action.owner = address(0);
    ...
    function replaceOwner(address owner, address newOwner) external {
        ...
        require(
            owner != msg.sender,
            "MultiSig: senders cannot replace themselves"
        );
        require(
            s_owners[newOwner] == false,
            "MultiSig: new owner must not exist"
        );
    ...
    function sig(bytes calldata data) public pure returns (bytes4) {
        if (data.length < 4) {
            return bytes4(0);
        }
        return (data[0] |
