...
it('EIP712: should be able to execute multi external calls: signer==operator, sender==owner', async () => {
    ...
    const { receipt: receiptERC20 } = await factoryProxy.batchMultiCall2(msgsERC20, 8, { from: activator, gasPrice: 200, gas: 1e6 }) // .catch(revertReason => console.log({ revertReason: JSON.stringify(revertReason, null ,2) }))
